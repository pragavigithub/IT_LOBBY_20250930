...
class SAPIntegration:
    ...
    def create_serial_item_stock_transfer(self, transfer_document):
        ...
        try:
            # Determine timeout based on transfer size - large volumes need more time
            item_count = len(transfer_document.items)
            if item_count > 500:
    ...
    def _validate_single_series(self, serial_number, item_code, warehouse_code=None):
        """Internal method to validate a single series without recursion"""
        if not self.ensure_logged_in():
        ...
        try:
            ...
            if warehouse_code:
                payload = {
                    "ParamList": f"series='{serial_number}'&whsCode='{warehouse_code}'&itemCode='{item_code}'"
                }
            else:
                payload = {
            ...
            if response.status_code == 200:
                data = response.json()
                logging.debug(f"SAP validation response for {serial_number}: {data}")
                ...
                if data.get('value') and len(data['value']) > 0:
                    ...
                    # Series not found in the specified warehouse - this means it's INVALID
                    return {
                        'valid': False,
                        'available_in_warehouse': False,
                        'error': f'Series {serial_number} not found in SAP or not available in warehouse {warehouse_code}',
                        'message': f'Series {serial_number} validation failed'
                    }
            else:
                logging.error(f"SAP API validation failed for {serial_number}: {response.status_code} - {response.text}")
                return {
                    'valid': False,
    ...
    def validate_series_with_warehouse(self, serial_number, item_code, warehouse_code=None):
        ...
        Args:
            ...
            warehouse_code: Optional warehouse code to check series availability in specific warehouse
        """
        # Use the internal method to avoid any potential recursion issues
        return self._validate_single_series(serial_number, item_code, warehouse_code)
    ...
    def _validate_batch_chunk(self, serial_batch, item_code, warehouse_code):
        """Validate a chunk of serial numbers using individual SAP B1 queries
        ...
        try:
            ...
            # Return error for all serials in this chunk
            for serial in serial_batch:
                results[serial] = {
                    'valid': False,
                    'error': f'Batch chunk validation error: {str(e)}',
                    'available_in_warehouse': False
                }
